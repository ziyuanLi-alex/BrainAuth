...
# Get project root directory (assuming organize_file.py is in the src directory)
PROJECT_ROOT = Path(__file__).parent.parent.absolute()

# Define relative paths
RAW_DIR = PROJECT_ROOT / "data" / "raw"
ORGANIZED_DIR = PROJECT_ROOT / "data" / "organized"
EYES_OPEN_DIR = ORGANIZED_DIR / "eyes_open"
EYES_CLOSED_DIR = ORGANIZED_DIR / "eyes_closed"

# Create directories
def create_directories():
    """Create necessary directory structure"""
    for dir_path in [ORGANIZED_DIR, EYES_OPEN_DIR, EYES_CLOSED_DIR]:
        dir_path.mkdir(exist_ok=True, parents=True)
    logger.info("Directories created successfully")


# Organize files for a single subject
def organize_subject_files(subject_dir):
    """
    Organize files for a single subject, organizing eyes-open and eyes-closed files

    Args:
        subject_dir: Path to the subject directory

    Returns:
        success_count: Number of files successfully organized
    """
    success_count = 0
...
def organize_all_files():
    """Organize files for all subjects"""
    create_directories()
    ...
    for subject_dir in tqdm(subject_dirs):
        total_subjects += 1
        success_count = organize_subject_files(subject_dir)
        total_success += success_count

    logger.info(f"Organizing complete. Organized {total_subjects} subjects, successfully copied {total_success} files.")
...
if __name__ == "__main__":
    ...
    logger.info(f"Organized data directory: {ORGANIZED_DIR}")
